 ...
 */
@SpringBootTest @AutoConfigureMockMvc class FraudAiApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("风控渠道"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanAssessFraudRisk()throws Exception{mvc.perform(post("/api/ai/fraud/assess").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"transactionNo\":\"TX-001\",\"amount\":18000,\"customerAverageAmount\":1200,\"transactionsLastHour\":11,\"deviceRiskScore\":82,\"locationRiskScore\":75,\"newPayee\":true,\"impossibleTravel\":true}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.decision").value("BLOCK")).andExpect(jsonPath("$.data.riskScore").isNumber());}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
